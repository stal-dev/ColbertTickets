...
export async function getPreviousState() {
  try {
    let state;
    if (useLocalStorage()) {
      console.log('[getPreviousState] Using local storage');
      state = await localRead(STATE_BLOB_NAME);
    } else {
      // Use Vercel Blob with custom token
      const blobInfo = await head(STATE_BLOB_NAME, getBlobConfig());
      if (!blobInfo) {
        console.log('[getPreviousState] No blob found, returning null');
        return null;
      }

      const response = await fetch(blobInfo.url);
      if (!response.ok) {
        console.log(`[getPreviousState] Blob fetch failed (${response.status}), returning null`);
        return null;
      }
    ...
    if (state) {
      console.log(`[getPreviousState] Loaded state: key="${state.stateKey}", checkedAt=${state.checkedAt}, avail=${state.totalAvailable}, wait=${state.totalWaitlist}`);
    } else {
      console.log('[getPreviousState] No previous state found');
    }
    return state;
  } catch (error) {
    console.log('[getPreviousState] Error loading state:', error.message);
    return null;
  }
}
...
export async function shouldSendEmail(result) {
  ...
  console.log(`\n========== shouldSendEmail Check ==========`);

  // --- Current state ---
  console.log(`[CURRENT STATE] key: ${currentKey}`);
  console.log(`[CURRENT STATE] checkedAt: ${result.checkedAt}`);
  console.log(`[CURRENT STATE] totalAvailable: ${currentTotalAvailable}, totalWaitlist: ${currentTotalWaitlist}`);
  result.shows.forEach(show => {
    const avail = show.availableDates || [];
    const wait = show.waitlistDates || [];
    if (avail.length > 0 || wait.length > 0) {
      ...
      console.log(`  [CURRENT] ${show.name}: Avail(${avail.length})[${aRange}], Wait(${wait.length})[${wRange}]`);
    } else {
      console.log(`  [CURRENT] ${show.name}: No tickets`);
    }
  });

  // --- Previous stored state ---
  if (previousState) {
    const keyChanged = previousState.stateKey !== currentKey;
    console.log(`[PREV STATE] key: ${previousState.stateKey}`);
    console.log(`[PREV STATE] checkedAt: ${previousState.checkedAt}`);
    console.log(`[PREV STATE] totalAvailable: ${previousState.totalAvailable}, totalWaitlist: ${previousState.totalWaitlist}`);
    console.log(`[PREV STATE] stateKey changed: ${keyChanged}`);
    if (keyChanged) {
      console.log(`  [KEY DIFF] prev: ${previousState.stateKey}`);
      console.log(`  [KEY DIFF] curr: ${currentKey}`);
    }
    if (previousState.summary) {
      previousState.summary.forEach(s => {
        console.log(`  [PREV] ${s.name}: Avail(${s.availableCount}), Wait(${s.waitlistCount}), firstAvail: ${s.firstAvailable || 'none'}, lastAvail: ${s.lastAvailable || 'none'}`);
      });
    }
  } else {
    console.log(`[PREV STATE] none (first run)`);
  }

  // --- Last email sent state ---
  if (lastSentEmail) {
    console.log(`[LAST EMAIL] sentAt: ${lastSentEmail.checkedAt}`);
    console.log(`[LAST EMAIL] totalAvailable: ${lastSentEmail.totalAvailable || 0}, totalWaitlist: ${lastSentEmail.totalWaitlist || 0}`);
    console.log(`[LAST EMAIL] reason: ${lastSentEmail.emailReason || 'unknown'}`);
    const availDelta = currentTotalAvailable - (lastSentEmail.totalAvailable || 0);
    const waitDelta = currentTotalWaitlist - (lastSentEmail.totalWaitlist || 0);
    console.log(`[DELTA vs LAST EMAIL] available: ${availDelta >= 0 ? '+' : ''}${availDelta}, waitlist: ${waitDelta >= 0 ? '+' : ''}${waitDelta}`);
  } else {
    console.log(`[LAST EMAIL] none (no email has been sent yet)`);
  }

  console.log(`===========================================\n`);

  if (process.env.ALWAYS_EMAIL === 'true') {
    const decision = { shouldEmail: true, reason: 'ALWAYS_EMAIL is enabled', isNewAvailability: false, stateChanged: previousState?.stateKey !== currentKey };
    console.log(`[DECISION] shouldEmail: ${decision.shouldEmail}, reason: ${decision.reason}`);
    return decision;
  }

  // If state hasn't changed since last stored state, nothing new to report
  if (previousState && previousState.stateKey === currentKey) {
    const decision = { shouldEmail: false, reason: 'State unchanged since last check', isNewAvailability: false, stateChanged: false };
    console.log(`[DECISION] shouldEmail: ${decision.shouldEmail}, reason: ${decision.reason}`);
    return decision;
  }
  ...
  if (!lastSentEmail) {
    if (currentTotalAvailable > 0 || currentTotalWaitlist > 0) {
      const decision = { shouldEmail: true, reason: 'Initial availability detected', isNewAvailability: true, stateChanged: true };
      console.log(`[DECISION] shouldEmail: ${decision.shouldEmail}, reason: ${decision.reason}`);
      return decision;
    }
    const decision = { shouldEmail: false, reason: 'No tickets available (initial run)', isNewAvailability: false, stateChanged: previousState?.stateKey !== currentKey };
    console.log(`[DECISION] shouldEmail: ${decision.shouldEmail}, reason: ${decision.reason}`);
    return decision;
  }
  ...
  if (currentTotalAvailable > lastTotalAvail) {
    const decision = { shouldEmail: true, reason: `Available tickets increased: ${lastTotalAvail} -> ${currentTotalAvailable}`, isNewAvailability: true, stateChanged: true };
    console.log(`[DECISION] shouldEmail: ${decision.shouldEmail}, reason: ${decision.reason}`);
    return decision;
  }

  if (currentTotalWaitlist > lastTotalWait) {
    const decision = { shouldEmail: true, reason: `Waitlist tickets increased: ${lastTotalWait} -> ${currentTotalWaitlist}`, isNewAvailability: true, stateChanged: true };
    console.log(`[DECISION] shouldEmail: ${decision.shouldEmail}, reason: ${decision.reason}`);
    return decision;
  }

  const decision = { shouldEmail: false, reason: 'No increase in ticket counts', isNewAvailability: false, stateChanged: previousState?.stateKey !== currentKey };
  console.log(`[DECISION] shouldEmail: ${decision.shouldEmail}, reason: ${decision.reason}`);
  return decision;
}