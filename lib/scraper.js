...
// Cache the executable path to avoid re-downloading
let cachedExecutablePath = null;
let downloadPromise = null;

async function downloadAndExtractChromium() {
  const extractPath = '/tmp/chromium-extracted';
  const execPath = `${extractPath}/chromium`;
  ...
  downloadPromise = (async () => {
    try {
      ...
      // Download from the deployed Vercel URL
      ...
      if (!response.ok) {
        throw new Error(`Failed to download chromium: ${response.status}`);
      }
      ...
      execSync(`chmod +x "${execPath}"`, { stdio: 'ignore' });

      cachedExecutablePath = execPath;
      console.log(`Chromium extracted to: ${execPath}`);
      return execPath;
    } catch (error) {
...
async function getBrowser() {
  ...
  const executablePath = await downloadAndExtractChromium();
