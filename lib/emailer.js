  ...
  const mailOptions = {
    ...
    html: `
      ...
      <ol style="margin-top: 0;">
        ${ticketInfo.shows.map(show => {
          let text;
          if (show.hasAvailable) {
            text = `<strong>${show.name}:</strong> 🎟️ ${show.availableDates.map(d => d.display || d.date).join(', ')}`;
          } else if (show.hasWaitlist) {
            text = `<strong>${show.name}:</strong> ⏳ ${show.waitlistDates.map(d => d.display || d.date).join(', ')}`;
          } else if (show.error) {
            text = `<strong>${show.name}:</strong> ❌ Error: ${show.error}`;
          } else {
            text = `<strong>${show.name}:</strong> ❌ Not available`;
          }
          return `<li>${text}</li>`;
        }).join('')}
      </ol>
...
Summary:
${ticketInfo.shows.map((show, i) => {
  let text;
  if (show.hasAvailable) {
    text = `${show.name}: 🎟️ ${show.availableDates.map(d => d.display || d.date).join(', ')}`;
  } else if (show.hasWaitlist) {
    text = `${show.name}: ⏳ ${show.waitlistDates.map(d => d.display || d.date).join(', ')}`;
  } else if (show.error) {
    text = `${show.name}: ❌ Error: ${show.error}`;
  } else {
    text = `${show.name}: ❌ Not available`;
  }
  return `${i + 1}. ${text}`;
}).join('\n')}
